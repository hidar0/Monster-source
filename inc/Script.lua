...
#      ███    ██▄ ███    ███          ███          ███ «Dev : @IIIZIIII
#      ███    ███ ███    ███    ▄█    ███    ▄█    ███
#    ▄█████████▀   ▀██████▀   ▄████████▀   ▄████████▀  «Source TH3Monster BY @SouurceLink
#---------------------------------------------------------------------
]]
...
return sendMsg(msg.chat_id_,msg.id_,"المالك :\n["..v.user.first_name.."](t.me/"..(v.user.username or "SouurceLink"))
end
end
...
sendMsg(msg.chat_id_,msg.id_,'المالك :\n['..info.username..'](http://t.me/SouurceLink)  \n')
end
...
if not msg.Admin then return "-«هذا الامر يخص {نائب المدير,المدير,المنشئ,المطور} فقط  \n" end
...
return sendMsg(arg.ChatID,arg.MsgID,"-*«* لا يمكنك تقييد النائب مدير\n🛠") 
...
return sendMsg(msg.chat_id_,msg.id_,"-*«* لا يمكنك تقييد النائب مدير\n🛠") 
end
...
sendMsg(arg.ChatID,arg.MsgID,"-«المستخدم  ⋙「 "..NameUser.." 」 \n📋«تم بالتأكيد رفعه نائب مدير  في المجموعه") 
else
redis:hset(Monster..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(Monster..'admins:'..arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,"-«المستخدم  ⋙「 "..NameUser.." 」 \n📋«تم رفعه نائب مدير  في المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
...
sendMsg(arg.ChatID,arg.MsgID,"-«المستخدم  ⋙「 "..NameUser.." 」 \n📋«تم بالتأكيد تنزيله نائب مدير  في المجموعه") 
else
redis:srem(Monster..'admins:'..arg.ChatID,arg.UserID)
sendMsg(arg.ChatID,arg.MsgID,"-«المستخدم  ⋙「 "..NameUser.." 」 \n📋«تم تنزيله نائب مدير  في المجموعه") 
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
elseif MsgText[2] and MsgText[2]:match('@[%a%d_]+') then
...
elseif redis:sismember(Monster..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"-*«* لا يمكنك طرد النائب مدير\n🛠") 
elseif  redis:sismember(Monster..'whitelist:'..arg.ChatID,UserID) then 
...
elseif redis:sismember(Monster..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"-*«* لا يمكنك حظر النائب مدير\n🛠") 
elseif  redis:sismember(Monster..'whitelist:'..arg.ChatID,UserID) then 
...
elseif redis:sismember(Monster..'admins:'..arg.ChatID,UserID) then 
return sendMsg(arg.ChatID,arg.MsgID,"-*«* لا يمكنك كتم النائب مدير\n🛠") 
elseif  redis:sismember(Monster..'whitelist:'..arg.ChatID,UserID) then 
...
return "-«بواسطه ⋙ "..msg.TheRankCmd.."   \n-«تم مسح {"..Admins.."} من النائبين في البوت \n✓"
end
...
if MsgText[1] == "مسح النائبين" then 
if not msg.Director then return "-«هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end

local Admins = redis:scard(Monster..'admins:'..msg.chat_id_)
if Admins == 0 then  
return "-*«* اوه ☢ هنالك خطأ -\n-«عذرا لا يوجد نائبين ليتم مسحهم ✓" 
end
...
sendMsg(arg.ChatID,arg.MsgID,'-*«* كلا البوت ليس نائب مدير في المجموعة -')
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
return false 
end
...
if MsgText[1] == "رفع النائبين" then
if not msg.Creator then return "-«هذا الامر يخص {المطور,المنشئ} فقط  \n" end
...
if UserID == 1099083018 or UserID == 60809019 then return sendMsg(msg.chat_id_,msg.id_,"-*«* لآ يمكنك تنفيذ الامر ضد مطور السورس \n❕") end
...
DonisDown = DonisDown.."❌«تم تنزيله من النائب مدير ✓️\n"
end 
...
if MsgText[1] == "اضف امر" or MsgText[1] == "تعين امر" or MsgText[1] == "اضف امر" then
if not msg.Director then return "-«هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n" end
if MsgText[2] then
...
return  • «اوه  لا يوجد كليشه مطور اصلا \n  end
...
if not msg.SudoBase then return "-«هذا الامر يخص {المطور الاساسي} فقط  \n" end
local GetVerison = https.request('https://hidar0.github.io/GetVersion.txt') or "0"
...
if MsgText[1] == 'جلب نسخه' then
if not msg.SudoBase then return"-«هذا الامر يخص {المطور الاساسي} فقط  \n" end
return buck_up_groups(msg)
end 

if MsgText[1] == 'رفع نسخه' then
if not msg.SudoBase then return "-«هذا الامر يخص {المطور الاساسي} فقط  \n" end
if msg.reply_id then
GetMsgInfo(msg.chat_id_,msg.reply_id,function(arg, data)
...
sendMsg(arg.chat_id_,arg.id_,'-*«* عذرا اسم الملف غير مدعوم للنظام او لا يتوافق مع سورس لينك يرجى جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n❕')
end  
else
...
local inline = {{{text="قنآهہ‏‏ آلسـورس : لـيـنـک",url="http://t.me/SouurceLink"}}}
send_key(msg.sender_user_id_,'  [قناة السورس : لـيـنـک](http://t.me/SouurceLink)',nil,inline,msg.id_)
return false
end
...
• اذاعه بالتثبيت
• اذاعه عام بالتوجيه
• جلب نسخه ↜ لجلب نسخه احتياطيه للمجموعات 
• رفع نسخه ↜ لرفع النسخه الاحتياطيه
• تعيين قائمه الاوامر
• مسح كلايش التعليمات
• تعيين كليشه ستارت
• تعيين ايدي عام
...
if msg.Admin then return "-*«* لا استطيع طرد المدراء والنائبين والمنشئين  \n" end
...
return [[
  ...
  ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
 (http://t.me/SouurceLink)[Link]
 
    [مطور السورس](http://t.me/IIIZIIII 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
if not msg.SudoBase then return "-«هذا الامر يخص {المطور الاساسي} فقط  \n" end
local Get_Files, res = https.request("https://hidar0.github.io/GetFiles.json")
print(Get_Files)
print(res)
if res == 200 then
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/hidar0/hidar0.github.io/master/plugins/"..FileName)
if res == 200 then
...
if msg.text ~= "مطور اساسي" and msg.text ~= "مطور" and msg.text ~= "منشئ اساسي" and msg.text ~= "منشئ" and msg.text ~= "مدير" and msg.text ~= "نائب مدير" and msg.text ~= "مميز" then
...
redis:del(Monster..":RtbaNew5:"..msg.chat_id_)
elseif msg.text == "نائب مدير" then
redis:del(Monster..":RtbaNew6:"..msg.chat_id_)
...
elseif rtbanamenew == "نائب مدير" then
...
if msg.reply_id then
sendMsg(msg.chat_id_,msg.id_,'اوك سيدي 🌝🍃')
return sendMsg(msg.chat_id_,msg.reply_id,'تعال هيوو  😒 شو طالعه عينك ولسانك طويل سربوت  اشو تعال بله شنو هاذ ويهك هاذ 😳 كول بربك , 😅 لك #دي وتفو بعد لتندك بتاج راسك خوش حمبقلبي  👍🏿') 
end
...
elseif msg.Admin then 
return sendMsg(msg.chat_id_,msg.id_,"فوك مامصعدك نائب مدير ؟؟ انته انجب 😏")
else 
